...
public class GetAllAppsFromRegistry {
    private static final List<String> ALL_APPS_FROM_REGISTRY =
            List.of("reg", "query", "HKEY_CURRENT_USER\\Software\\Valve\\Steam\\apps");
    ...
    public static List<Long> get() {
        ...
        return perLineOutput.stream()
                .map(
                        // e.g. HKEY_CURRENT_USER\Software\Valve\Steam\apps\1005300 => 1005300
                        line -> Long.parseLong(
                                Iterables.getLast(Splitter.on("\\").splitToList(line))))
                .toList();
    }
}
